...
int clglSharingIsSupported(cl_device_id device_id)
{
  ...
  std::cerr << "[cl_core]: GL sharing support " << glSupported << std::endl;
...
std::string LoadAndDecrypt(const std::string& ecrypted)
{
  ...
  for (int i = resStr.size() - 32; i < resStr.size(); i++)
    resStr[i] = '\0';
...
CLProgram::CLProgram(cl_device_id a_devId, cl_context a_ctx, 
                     ...
                     const std::string& encrypted, const std::string& binPath) : m_refCounter(0)
{
  m_ctx = a_ctx;
  m_dev = a_devId;
  ...
  if (!loadBinarySuccess)
  {
    ...
    if (encrypted != "")
    {
      ...
      if (encrypted == "crypt")
      {
        ...
        {
          int n = (computeSource.size() / 8) * 8 + 8 - computeSource.size();
